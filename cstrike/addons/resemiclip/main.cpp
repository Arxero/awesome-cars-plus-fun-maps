...
inline bool IsTeamAllowed(CBasePlayer *HostPlayer, CBasePlayer *EntPlayer)
{
	...
	// Ghosts use the spectator team. Let any player pass through them while
	// preserving the configured collision rule for active T and CT players.
	if (hostTeam == SPECTATOR || EntPlayer->m_iTeam == SPECTATOR || g_Config.GetTeam() == SC_TEAM_ALL)
		return true;
