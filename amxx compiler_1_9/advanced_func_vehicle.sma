...
new Float:vtolVelocity = 200.0;
new Float:NOSVelocity = 100.0;
...
public vehicleThink(id) {
	new Float:launchTime;
	launchTime = get_ent_data_float(id, "CFuncVehicle", "m_flLaunchTime");
	if (launchTime != -1) {
		new Float:timeSinceLaunch = get_gametime() - launchTime;
		set_ent_data_float(id, "CFuncVehicle", "m_flLaunchTime", launchTime + (timeSinceLaunch / 40)); // divide less = add more = go further
	}
}
	...
	if (is_user_alive(id) && cs_get_user_driving(id) > 0 && cs_get_user_team(id) != CS_TEAM_SPECTATOR) {
		...
		if ((Button & IN_ATTACK2) && !(OldButtons & IN_ATTACK2)) {
			switch (vWeapon) {
				...
				case VWEAPON_HORN: {
					if (get_gametime() > vehicleLastShootTime[vIndex] + 2.0) {
				...
				case VWEAPON_SHIP_HORN: {
					if (get_gametime() > vehicleLastShootTime[vIndex] + 5.0) {
...
public checkDelay(id, vIndex) {
	if (get_gametime() < vehicleLastShootTime[vIndex] + 5.0) {
  		client_print(id,print_chat, "[AFV] Try again in %d seconds",floatround( vehicleLastShootTime[vIndex] + 5.0 - get_gametime()+ 1));
		return false;
	} else {
		return true;
	}
