...
public plugin_init( )
{
	...
	register_plugin( "VolleyBall Mod", PLUGIN_VERSION, "tuty, luki1412" );
	...
	new CMap[32];
	get_mapname(CMap, 31);
	if(contain(CMap, "vb_") == -1)
		set_fail_state("The current map is not a valid VolleyBall map(doesnt start with vb_) !");
}
...
public GiveBallToPlayer( taskid )
{
	...
	if(!is_user_connected( iRandomPlayer ) || !is_user_alive( iRandomPlayer ) ){
		UTIL_GiveBallRandom( );
	...
	new szName[ 32 ];
	get_user_name( iRandomPlayer, szName, charsmax( szName ) );
	new Teami = get_user_team( iRandomPlayer );
	...
	set_pev( iEntity, pev_iuser2, Teami );
...
public ForwardBallTouchPlayer( iPlayer, iBall )
{
	...
	switch( iTeam )
	{
		case 1:
		{
			...
			UTIL_BeamFollow( iBall, 255, 50, 50 );

			set_rendering( iBall, kRenderFxGlowShell, 255, 50, 50, kRenderNormal, 25 );
		}
		
		case 2:
		{
			UTIL_KillBeamFollow( iBall );
			UTIL_BeamFollow( iBall, 50, 50, 255 );

			set_rendering( iBall, kRenderFxGlowShell, 50, 50, 255, kRenderNormal, 25 );
		}
	}
...
public ForwardPlayerTouchTerritory( iPlayer, iEntity )
{
	...
	if( equal( szTeritoryName, "CT", 2 ) )
	{
		if( Teami == 1 )
		{
			...
			set_hudmessage( 255, 50, 50, -1.0, 0.86, 1, 6.0, 5.0 );
	...
	if( equal( szTeritoryName, "T", 1 ) )
	{
		if( Teami == 2 )
		{
			...
			set_hudmessage( 50, 50, 255, -1.0, 0.86, 1, 6.0, 5.0 );
...
{
	forward_return( FMV_STRING, "VolleyBall" );
...
stock UTIL_SlayTeam( const szTeam[ ] )
{
	...
	get_players( iPlayers, iCount, "e", szTeam );
