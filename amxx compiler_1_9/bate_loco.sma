...
new g_Datos[33][DATOS_ALL];

new g_RondaFinish
new g_ConteoRevivir[33]
new g_ConteoInmunidad[33]
new g_MaxPlayers
...
public Forward_PlayerKilled(Victima, Atacante, shouldgib)
{
	if(!is_user_connected(Atacante) && g_Bateado[Victima])
	{
		...
		FDCC(g_Bateado[Victima], TEAM_COLOR, "%s Mataste a ^4%s ^1de un bataso.", PREFIXBL, Nombre_Victima)
		FDCC(Victima, TEAM_COLOR, "%s El bateador ^4%s ^1te ha matado de un bataso.", PREFIXBL, Nombre_Atacante)
	}
...
{			
	...
	{
		FDCC(id, TEAM_COLOR, "%s Ya no podras ser revivido, No tienes vidas.", PREFIXBL)
		remove_task(id+TASK_REVIVIR)
		return PLUGIN_HANDLED
	}
	...
	if(!g_ConteoRevivir[id])
	{
		...
		FDCC(id, TEAM_COLOR, "%s Fuiste Revivido, Tenes una vida menos.", PREFIXBL)
		g_PlayerInmune[id] = true
		ExecuteHamB(Ham_CS_RoundRespawn, id);
		ScreenFade(id, 5.0, szRed, szGreen, szBlue, 120)
...
public DhudInfo(id) 
{
	...
	if(!is_user_alive(id))
	{
		Spect = pev(id, pev_iuser2);
		get_user_name(Spect, Name_Spect, charsmax(Name_Spect))
		
		if(!Spect)
		{
			set_dhudmessage(255, 255, 250, -1.0, 0.04, 1, 8.0, 0.8, 5.0);
			show_dhudmessage (id, "BATE LOCO")

			set_dhudmessage(250, 200, 0, -1.0, 0.10, 1, 8.0, 0.8, 5.0);
			show_dhudmessage (id, "ANTI-TERRORISTAS [ %d ] | [ %d ] TERRORISTAS^n[ VIDAS: %d/2 ]", g_RoundWinCT, g_RoundWinT, g_Datos[id][VIDAS]);
		}
		else
		{
			set_dhudmessage(255, 255, 250, -1.0, 0.14, 1, 8.0, 0.8, 5.0);
			show_dhudmessage (id, "BATE LOCO^nEspecteando a JUGADOR: %s", Name_Spect)

			set_dhudmessage(250, 200, 0, -1.0, 0.22, 1, 8.0, 0.8, 5.0);
			show_dhudmessage (id, "ANTI-TERRORISTAS [ %d ] | [ %d ] TERRORISTAS^n[ VIDAS: %d/2 ]", g_RoundWinCT, g_RoundWinT, g_Datos[Spect][VIDAS]);
		}
	}
	else
	{
...
{	
	...
	if(get_pcvar_num(gp_CvarNoKillNoName))
	{
		if(!equal(name,newname))
		{
			...
			FDCC(id, TEAM_COLOR, "%s El cambio de tag esta deshabilitado.", PREFIXBL);
			return FMRES_IGNORED;
		}
	}
	return FMRES_IGNORED;
...
public Forward_ClientKill(id)
{
	...
	if(get_pcvar_num(gp_CvarNoKillNoName))
	{
		FDCC(id, TEAM_COLOR, "%s No podes suicidarte en estos momentos", PREFIXBL)
...
public Forward_PlayerSpawn(id)
{
	...
	switch(random_num(0, 15))
	{
		case 3, 9, 12:
		{
			FDCC(id, TEAM_COLOR, "%s MiniMOD ^4BATE LOCO^1 creado por: ^4%s ^1- version: ^4%s^1.", PREFIXBL, INFOPLUGIN[2], INFOPLUGIN[1])
		}
	}
	return HAM_IGNORED 
}
